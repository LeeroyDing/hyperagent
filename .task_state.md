...
- [ ] **Task 1.1: CLI Framework**: Integrate `spf13/cobra` in `main.go` to replace standard flags with a subcommand structure.
...
- [ ] **Automated**: Integration test script: `hyperagent up && hyperagent status && hyperagent down` verifying exit codes and process existence.
- [ ] **Manual**: Run `hyperagent up`, verify process with `ps aux | grep hyperagent`.
- [ ] **Manual**: Verify `~/.hyperagent/hyperagent.pid` contains the correct PID.
